...
with DAG(
    dag_id='pipeline_enem',
    schedule_interval=None,
    start_date=datetime(2023, 1, 20),
    catchup=False,
    max_active_runs=1,
    tags=['enem', "kubernetes-pod-operator", 'spark-operator', 'k8s'],
) as dag:
# [END instantiate_dag]
    ...
    ingestion = KubernetesPodOperator(
          ...
          pod_template_file=f"{DAGS_FOLDER_PATH}/pipeline-enem-ingestion.yaml",
          in_cluster=True,
          get_logs=True
      )
    ...
    processing = SparkKubernetesOperator(
        task_id='processing',
        namespace=SPARK_NAMESPACE,
        application_file='pipeline-enem-processing.yaml',
        do_xcom_push=True
    )
